...
- Серверная обработка фото: GPS EXIF считывается до очистки, поддерживаемое
  изображение перекодируется в WebP без метаданных, уменьшается до лимита и
  получает UUID-имя и SHA-256 безопасной копии.
- ADR о приватном quarantine для исходников и публикации только обработанных
  копий.
...
- `POST /api/v1/reports/` — авторизованный владелец создаёт активное
  объявление и животное одной транзакцией. Валидируются координаты и время,
  забаненный пользователь получает отказ.
- Инвариант 5 на уровне API: телефон не публикуется даже в ответе владельцу.
